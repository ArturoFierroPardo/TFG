...
MODELS_CONFIG = {
    ...
    "gemma":  "arturofierrop2_c6c6/google/gemma-2-9b-it-50323051",
}
...
PRECIOS = {
    "meta-llama/llama-3.2-3b-instruct":                          {"input": 0.00000006, "output": 0.00000010},
    "Qwen/Qwen2.5-7B-Instruct-Turbo":                           {"input": 0.00000020, "output": 0.00000020},
    "arturofierrop2_c6c6/google/gemma-2-9b-it-50323051":         {"input": 0.00000020, "output": 0.00000020},
}
PRECIO_DEFAULT = {"input": 0.0000002, "output": 0.0000002}

# kWh/token estimado para cálculo de CO2
KWH_POR_TOKEN = {
    "meta-llama/llama-3.2-3b-instruct":                          0.00000008,
    "Qwen/Qwen2.5-7B-Instruct-Turbo":                           0.0000002,
    "arturofierrop2_c6c6/google/gemma-2-9b-it-50323051":         0.0000002,
}
CO2_POR_KWH = 475
...
def run_pipeline(model_key, limit=None, csv_path=DATASET_PATH):
    ...
    pares = cargar_dataset_teleco(csv_path, limit)
    if pares is None:
        return
    ...
    if len(preds_validas) > 0:
        # Globales (con try/except para no perder todo si una métrica falla)
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", required=True, choices=list(MODELS_CONFIG.keys()),
                        help="Modelo a usar: llama (3B), qwen (7B), gemma (9B)")
    ...
    if args.fix:
        ...
        print(f"\nPipeline OpenRouter SLM: TELECO + {args.model.upper()}", end="")
        if args.limit:
            print(f" | Límite: {args.limit} filas")
        else:
            print(" | Dataset completo")
        run_pipeline(args.model, args.limit, args.csv)
